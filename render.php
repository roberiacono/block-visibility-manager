...
// Time
// Check if time-based visibility is enabled and apply time range logic
if ( ! empty( $attributes['bvmEnableTime'] ) && $attributes['bvmEnableTime'] ) {
    $now = current_time( 'H:i' ); // WordPress time in 24-hour format
    //print_r('debug '.$now);
...
// Date
// Check if date-based visibility is enabled and apply date range logic
...
// If visibility is off, return empty string (hide block)
if ( ! $visible ) {
    return '';
}
...
$classes = '';
if (  !empty($attributes['bvmHideOnMobile'])) $classes .= ' hide-mobile';
if (!empty($attributes['bvmHideOnTablet'])) $classes .= ' hide-tablet';
if (!empty($attributes['bvmHideOnDesktop'])) $classes .= ' hide-desktop';
...
// Output
echo '<div class="' . esc_attr(trim($classes)) . '">' . $block_content . '</div>';
